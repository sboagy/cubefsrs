...
- **test users:** Reuse TuneTrees users (alice…iris @tunetrees.test) as shared workspace test identities; in local dev both apps authenticate against one shared Supabase instance, while in CI each app workflow gets its own isolated Supabase instance. The shared auth bootstrap and Playwright auth-state refresh flow are owned by Rhizome, while CubeFSRS only consumes those users and their credentials (`TestPassword123!` from `ALICE_TEST_PASSWORD`)
...
9. **`e2e/setup/auth.setup.ts`** — runs once before all tests:
   ...
   - Reads `ALICE_TEST_PASSWORD` env var (same as TuneTrees; value `TestPassword123!`) — all 8 shared workspace test users share this password
...
| 2 | Test user passwords | Reuse `TestPassword123!` from `ALICE_TEST_PASSWORD` env var for the shared workspace test identities |
